...
class LogParserTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->logParser = new LogParser(array($this->getPathToTestFile()));
    }
    ...
    public function test_getLogLines_canMergeMultipleLogFiles()
    {
        $countOneFile = count($this->logParser->getLogLines());
        $this->assertGreaterThan(100, $countOneFile);

        $logParser = new LogParser(array($this->getPathToTestFile(), $this->getPathToTestFile()));
        $lines     = $logParser->getLogLines();

        $this->assertCount($countOneFile * 2, $lines);
    }
    ...
    private function getPathToTestFile()
    {
        return __DIR__ . '/../data/access.log';
    }
